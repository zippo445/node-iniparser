...
function unquote (a_value){
    if ((a_value==null) || (a_value.length<2)){
        return a_value;
    }

    if ( (a_value[0]=='"') && (a_value[a_value.length-1]=='"')){
        return a_value.substr(1,a_value.length-2);
    }
...
function parse(data){
	...
	lines.forEach(function(line){
		if(regex.comment.test(line)){
			...
			if(section){
				value[section][match[1]] = unquote(match[2]);
			}else{
				value[match[1]] = unquote(match[2]);
			}
